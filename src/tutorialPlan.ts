...
export type TutorialRewardKind = 'tokens' | 'prize' | 'nothing' | 'minigame' | 'streak_boost' | 'freePlays' | 'bonus';

export interface TutorialForcedReward {
  kind: TutorialRewardKind;
  amount?: number;          // for tokens or streak_boost
  prizeId?: string;         // for a specific prize (if you have an ID system later)
  minigame?: 'slot' | 'spin_wheel' | 'lootbox'; // force-open a minigame as part of the day (optional)
  notes?: string;           // designer note
  // Forced minigame outcome controls (optional, used only when kind==='minigame'):
  // Slot: force all three symbols to this emoji (or provide three via forceSlotSymbols[] for variety)
  forceSlotSymbol?: string;
  forceSlotSymbols?: [string,string,string];
  // Spin wheel: force the landed reward by matching label OR kind+amount
  forceSpinWheelLabel?: string;
  forceSpinWheelKind?: 'tokens'|'freePlays'|'cash'|'bonus'|'nothing';
  forceSpinWheelAmount?: number; // match against reward.amount when provided
  // Loot box: force the final reward similarly
  forceLootboxLabel?: string;
  forceLootboxKind?: 'tokens'|'freePlays'|'cash'|'bonus'|'nothing';
  forceLootboxAmount?: number;
}

export interface TutorialDayPlan {
  day: number;              // 1..8
  forcedDice: number;       // value the dice should show (1..6) or 0 to skip movement
  targetLevel: number;      // absolute level index we want to end on after movement (sanity-checked at runtime)
  reward: TutorialForcedReward; // reward granted after movement (before any category auto-trigger overrides)
  autoTriggerCategory?: boolean; // if false, suppress category auto trigger for this landing; default true
  forceLandingCategory?: 'mystery' | 'bonus_round' | 'instant_prize' | 'instant_tokens' | 'reveal' | 'minigame'; // override the landed tile's category to this before triggering it
  overrideZone?: 'carnival' | 'jungle'; // zone lock for the day (carnival for days 1-8, then jungle afterwards)
  transitionAfter?: boolean; // if true, after completing this day, switch to next zone rules
  designerNotes?: string;    // freeform notes for narrative/UI scripting
  popups?: Partial<Record<'start_day'|'pre_roll'|'post_roll'|'post_move'|'pre_minigame'|'post_minigame'|'post_instantwin'|'post_bonus_round'|'end_day', string|string[]>>; // inline popup definitions (start_day triggers right after day label fade-in / initial load)
}
...
export const TUTORIAL_DAYS: TutorialDayPlan[] = [
  {
    day: 1,
    forcedDice: 4,
    targetLevel: 5,
...
export function isTutorialActive(currentDay: number) {
  return currentDay >= 1 && currentDay <= 8;
}

export function getTutorialPlanForDay(day: number): TutorialDayPlan | undefined {
...
export function registerTutorialPopups(enqueue:(day:number, phase:string, html:string)=>void){
  TUTORIAL_DAYS.forEach(plan=>{
    if(!plan.popups) return;
    Object.entries(plan.popups).forEach(([phase, value])=>{
      if(!value) return;
      if(Array.isArray(value)) value.forEach(v=> enqueue(plan.day, phase, v));
      else enqueue(plan.day, phase, value);
    });
  });
}
